...
**作用**<br>
为了简化依赖注入，实现依赖注入，需要先创建所依赖的对象，而如果依赖的层级不叫多的时候，一级一级的创建将会变得十分麻烦， 容器就很好的解决了创建对象的问题
...
- @since 2.0.11 */ public function setDefinitions(array $definitions) { foreach ($definitions as $class => $definition) {
...
- @since 2.0.11 */ public function setSingletons(array $singletons) { foreach ($singletons as $class => $definition) {
